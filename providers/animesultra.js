 ...
 * Generated: 2026-09-11T18:14:24.564464516Z
 */
var __provider = (() => {
  ...
  function detectSeason(title, url) {
    ...
    const urlSeason = ((_a = url.match(/saison[-\s]*(\d+)/i)) == null ? void 0 : _a[1]) || ((_b = url.match(/cour[-\s]*(\d+)/i)) == null ? void 0 : _b[1]);
  ...
  function extractStreams(tmdbId, mediaType, season, episode) {
    return __async(this, null, function* () {
      ...
      const targetEpisodes = yield resolveTargetEpisodes(tmdbId, mediaType, season, epNum, { startTime, budgetMs: BUDGET_MS });
      let matches = [];
      const seenIds = /* @__PURE__ */ new Set();
      const trySearch = (title) => __async(null, null, function* () {
        var _a2;
        ...
        const results = yield searchAnime(cleanTitle);
        if (results && results.length > 0) {
          for (const r of results) {
            const id = (_a2 = r.url.match(/\/(\d+)-/)) == null ? void 0 : _a2[1];
      ...
      yield Promise.allSettled(searchPromises);
      if (matches.length === 0 && effectiveSeason && !isBudgetExhausted(startTime, BUDGET_MS)) {
        ...
        if (seasonQuery) {
          yield trySearch(`${seasonQuery} Saison ${effectiveSeason}`);
          yield trySearch(`${seasonQuery} Season ${effectiveSeason}`);
        }
      }
      ...
      const getFullStory = (newsId) => __async(null, null, function* () {
        if (fullStoryCache[newsId]) return fullStoryCache[newsId];
        try {
          const sf = yield safeFetch(`${BASE_URL}/engine/ajax/full-story.php?newsId=${newsId}`, {
            timeout: 1e4,
            headers: { "User-Agent": "Mozilla/5.0", "X-Requested-With": "XMLHttpRequest" }
          });
          if (sf) {
            const d = yield sf.json();
            if (d && d.html) {
          ...
          console.warn(`[AnimesUltra] Full-story fetch/parse failed for ${newsId}: ${e == null ? void 0 : e.message}`);
        }
        return null;
      });
      ...
      const fetchEpisodeServers = (epHref, $context, lang) => __async(null, null, function* () {
        const epRes = yield safeFetch(epHref, { timeout: 1e4, headers: { "User-Agent": "Mozilla/5.0" } });
        if (!epRes || !epRes.ok) {
      ...
      if (streams.length === 0 && effectiveSeason && matches.length > 1 && !isBudgetExhausted(startTime, BUDGET_MS)) {
        ...
        for (const m of matches) {
          ...
          const pNum = parseInt((_a = m.title.match(/(?:partie|part)\s*(\d+)/i)) == null ? void 0 : _a[1], 10) || 1;
          if (sNum !== effectiveSeason) continue;
          const nId = (_b = m.url.match(/\/(\d+)-/)) == null ? void 0 : _b[1];
          if (!nId) continue;
          const mLang = detectLang(m.title) || "vostfr";
          const key = `${pNum}-${mLang}`;
          if (!byPart[key]) {
            const html = yield getFullStory(nId);
            if (html) {
        ...
        for (const up of uniqueParts) {
          ...
          const $ref = import_cheerio_without_node_native2.default.load(allLang[0].html);
          ...
          if (targetLocal.length > 0) {
            for (const lv of allLang) {
              const $c = import_cheerio_without_node_native2.default.load(lv.html);
              const epHrefs = [];
              $c(".ep-item").each((i, el) => {
      ...
      const resolvedArray = yield Promise.all(streamPromises);
