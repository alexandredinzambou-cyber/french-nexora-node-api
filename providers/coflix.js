 ...
 * Generated: 2026-09-23T21:45:12.009911975Z
 */
var __provider = (() => {
  ...
  function flareFetchText(_0) {
    return __async(this, arguments, function* (url, opts = {}) {
      const base = flaresolverrBase();
      if (!base) return null;
      try {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), (opts.maxTimeout || 6e4) + 5e3);
        const res = yield fetch(base + "/v1", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            cmd: "request.get",
            url,
            maxTimeout: opts.maxTimeout || 6e4,
        ...
        if (data.status !== "ok" || !data.solution) {
          console.log(`[FlareSolverr] \xC9chec (${data.status}) pour ${url}`);
          return null;
        }
  ...
  function isCloudflareBlock(text) {
    ...
    return text.includes("error code: 1010") || text.includes("cf-browser-verification") || text.includes("Just a moment") || text.includes("Checking your browser") || text.length < 20 && text.length > 0;
  }
  function fetchFromDomain(_0, _1) {
    return __async(this, arguments, function* (domain, path, options = {}) {
      var _a, _b, _c, _d, _e, _f;
      const url = `https://${domain}${path}`;
      const isJson = options.responseType === "json";
      const mergedHeaders = __spreadValues(__spreadValues({}, isJson ? AJAX_HEADERS : HEADERS2), options.headers || {});
      ...
      for (let attempt = 0; attempt <= ((_a = options.retries) != null ? _a : 1); attempt++) {
        try {
          ...
          if (!res) {
            ...
            if (attempt < ((_c = options.retries) != null ? _c : 1)) yield sleep(RETRY_DELAYS[attempt] || 2e3);
            continue;
          }
          const text = yield res.text();
          if (isCloudflareBlock(text)) {
            console.log(`[Coflix] Cloudflare block on ${domain} (attempt ${attempt + 1})`);
            if (attempt < ((_d = options.retries) != null ? _d : 1)) yield sleep(RETRY_DELAYS[attempt] || 2e3);
            continue;
          }
          if (!res.ok) {
            if (res.status === 404) return isJson ? null : "";
            console.log(`[Coflix] HTTP ${res.status} on ${domain}`);
            if (attempt < ((_e = options.retries) != null ? _e : 1)) yield sleep(RETRY_DELAYS[attempt] || 2e3);
            continue;
          }
          if (isJson) {
            try {
          ...
          if (attempt < ((_f = options.retries) != null ? _f : 1)) yield sleep(RETRY_DELAYS[attempt] || 2e3);
        }
      }
      return null;
    });
  ...
  function fetchText(_0) {
    return __async(this, arguments, function* (path, options = {}) {
      for (const domain of DOMAINS) {
        const result = yield fetchFromDomain(domain, path, __spreadProps(__spreadValues({}, options), { responseType: "text" }));
        if (result) return result;
      }
      const absolute = path.startsWith("http") ? path : `https://${DOMAINS[0]}${path}`;
      const html = yield flareFetchText(absolute, { maxTimeout: 5e4 });
  ...
  function fetchJson(_0) {
    return __async(this, arguments, function* (path, options = {}) {
      for (const domain of DOMAINS) {
        const result = yield fetchFromDomain(domain, path, __spreadProps(__spreadValues({}, options), { responseType: "json" }));
        if (result) return result;
      }
      const absolute = path.startsWith("http") ? path : `https://${DOMAINS[0]}${path}`;
      const html = yield flareFetchText(absolute, { maxTimeout: 5e4 });
  ...
  var rateLimit, DOMAINS, HEADERS2, AJAX_HEADERS, RETRY_DELAYS;
  var init_http = __esm({
    "src/coflix/http.js"() {
      init_resolvers();
      init_flaresolverr();
      rateLimit = createProviderRateLimiter();
      DOMAINS = ["coflix.esq", "coflix.to", "coflix.fr", "coflix.blog"];
      HEADERS2 = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Safari/537.36",
  ...
  function toSlug(title) {
    ...
    return clean.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[':!.,?()\[\]"]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
  }
  ...
  function searchViaWpApi(query, mediaType, season, episode) {
    return __async(this, null, function* () {
      ...
      for (const post of posts) {
        ...
        const title = (((_a = post.title) == null ? void 0 : _a.rendered) || "").toLowerCase();
        ...
        if (mediaType === "tv" && season && episode) {
          ...
          for (const epPath of patterns) {
            const html2 = yield fetchText(epPath, { timeout: PAGE_TIMEOUT });
            if (html2 && html2.length > 100) {
              const iframeUrl = extractIframeUrl(html2);
              if (iframeUrl) {
                console.log(`[Coflix] WP search found episode: ${epPath}`);
                return { url: iframeUrl, lang: extractLanguage(html2, epPath) };
              }
            }
          }
        }
        ...
        const html = yield fetchText(moviePath, { timeout: PAGE_TIMEOUT });
  ...
  function probeMovie(slug) {
    return __async(this, null, function* () {
      return withCache(`movie_${slug}`, () => __async(null, null, function* () {
        const patterns = [`/film/${slug}/`, `/movie/${slug}/`];
        for (const path of patterns) {
          const html = yield fetchText(path, { timeout: PAGE_TIMEOUT });
          if (html && html.length > 200) {
            const iframeUrl = extractIframeUrl(html);
            if (iframeUrl) {
        ...
        return null;
      }), { successTtl: 3e5, failureTtl: 3e4 });
    });
  }
  function probeEpisode(slug, season, episode) {
    return __async(this, null, function* () {
      return withCache(`ep_${slug}_${season}_${episode}`, () => __async(null, null, function* () {
        const patterns = [
  ...
  function extractStreams(tmdbId, mediaType, season, episode) {
    return __async(this, null, function* () {
      ...
      if (mediaType === "movie") {
        for (const title of titles.slice(0, 3)) {
          ...
          for (const slug of [...new Set(candidates)]) {
            const result = yield probeMovie(slug);
            if (result) {
        ...
        const wpResult2 = yield searchViaWpApi(titles[0], mediaType, null, null);
        if (wpResult2) {
          const stream = toStream(wpResult2.url, wpResult2.lang, "Coflix", "https://coflix.esq");
          const resolved = yield resolveStream(stream);
          if (resolved && resolved.url) return [__spreadProps(__spreadValues({}, resolved), { provider: "coflix" })];
          return [__spreadProps(__spreadValues({}, stream), { provider: "coflix", type: "embed" })];
        }
      ...
      const targetEpisodes = yield resolveTargetEpisodes(tmdbId, mediaType, targetSeason, targetEpisode);
      for (const ep of targetEpisodes) {
        for (const title of titles.slice(0, 4)) {
          const candidates = generateSlugCandidates(title, targetSeason, null);
          for (const slug of [...new Set(candidates)]) {
            const result = yield probeEpisode(slug, targetSeason, ep);
            if (result) {
              const stream = toStream(result.url, result.lang, "Coflix", "https://coflix.esq");
              const resolved = yield resolveStream(stream);
              if (resolved && resolved.url) return [__spreadProps(__spreadValues({}, resolved), { provider: "coflix" })];
        ...
        if (ep !== targetEpisode) {
          for (const title of titles.slice(0, 2)) {
            const candidates = generateSlugCandidates(title, targetSeason, null);
            for (const slug of [...new Set(candidates)]) {
              const result = yield probeEpisode(slug, targetSeason, targetEpisode);
              if (result) {
                const stream = toStream(result.url, result.lang, "Coflix", "https://coflix.esq");
                const resolved = yield resolveStream(stream);
                if (resolved && resolved.url) return [__spreadProps(__spreadValues({}, resolved), { provider: "coflix" })];
      ...
      const wpResult = yield searchViaWpApi(titles[0], mediaType, targetSeason, targetEpisode);
      if (wpResult) {
  ...
  var init_extractor = __esm({
    "src/coflix/extractor.js"() {
      ...
      init_metadata();
      init_cache();
      withCache = createCache("cf", "Coflix");
      PAGE_TIMEOUT = 8e3;
    }
  });
  ...
  var require_index = __commonJS({
    "src/coflix/index.js"(exports, module) {
      init_extractor();
      init_resolvers();
