 ...
 * Generated: 2026-09-11T18:14:25.184465143Z
 */
var __provider = (() => {
  ...
  function isCloudflareBlock(text) {
    ...
    return text.includes("Cloudflare") || text.includes("cf-browser-verification") || text.includes("challenge-form") || text.includes("Attention Required") || text.includes("Just a moment...") || text.includes("jsd/main") || text.includes("cdn-cgi/bm/cv/") || /Ray ID: [a-f0-9-]{20,}/.test(text);
  }
  function fetchText(_0) {
    return __async(this, arguments, function* (url, options = {}) {
      var _b, _c, _d, _e, _f, _g;
      yield rateLimit(DOMAIN);
      const _a = options, { headers: customHeaders, method, timeout } = _a, rest = __objRest(_a, ["headers", "method", "timeout"]);
      const resolvedMethod = method || "GET";
      const maxRetries = resolvedMethod === "HEAD" ? 1 : (_b = options.retries) != null ? _b : 2;
      const mergedHeaders = __spreadValues(__spreadValues({}, HEADERS2), customHeaders || {});
      let lastError = null;
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        if (attempt > 0) {
          const delay = RETRY_DELAYS2[attempt - 1] || 4e3;
        ...
        try {
          ...
          lastError = e;
          if (attempt < maxRetries && (((_c = e.message) == null ? void 0 : _c.includes("fetch failed")) || ((_d = e.message) == null ? void 0 : _d.includes("NetworkError")) || ((_e = e.message) == null ? void 0 : _e.includes("timeout")) || ((_f = e.message) == null ? void 0 : _f.includes("no response")) || ((_g = e.message) == null ? void 0 : _g.includes("Cloudflare")))) {
            continue;
          }
  ...
  var HEADERS2, rateLimit, DOMAIN, RETRY_DELAYS2;
  var init_http = __esm({
    "src/voiranime/http.js"() {
      init_resolvers();
      HEADERS2 = {
  ...
  function batchProbe(urls, batchSize = 5, delayMs = 0) {
    return __async(this, null, function* () {
      ...
      for (let i = 0; i < urls.length; i += batchSize) {
        ...
        if (delayMs > 0 && i + batchSize < urls.length) yield sleep(delayMs);
      }
      return results;
    });
  }
  ...
  function searchAnime(title, season = 1) {
    return __async(this, null, function* () {
      ...
      if (results.length === 0 && !isProbeBudgetExhausted()) {
        ...
        const compactWords = title.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[':!.,?()\[\]]/g, "").replace(/[^a-z0-9\s]/g, "").trim().split(/\s+/).filter(Boolean);
        ...
        if (uniqueAltSlugs.length > 0) {
          ...
          const validAltUrls = yield batchProbe(altUrls, 2, 200);
            ...
            if (r.status === "fulfilled") {
              for (const res of r.value) {
                if (!results.some((ex) => ex.url === res.url)) {
                  results.push(__spreadProps(__spreadValues({}, res), { title: `${res.title}` }));
                }
              }
            }
          }
    ...
    return __async(this, null, function* () {
      try {
        ...
        if (filteredHosts.length === 0) {
          const $ = import_cheerio_without_node_native2.default.load(epRawHtml);
          let iframe = null;
          $("iframe").each((_, el) => {
  ...
  function extractStreams(tmdbId, mediaType, season, episode) {
    return __async(this, null, function* () {
      ...
      const resolvedEps = yield resolveTargetEpisodes(tmdbId, mediaType, season, episode, { startTime, budgetMs: BUDGET_MS });
      let targetEpisodes = resolvedEps;
      if (resolvedEps.length > 1) {
        targetEpisodes = [resolvedEps[1]];
      }
