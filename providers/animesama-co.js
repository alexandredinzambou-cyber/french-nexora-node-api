 ...
 * Generated: 2026-09-11T18:14:24.516464414Z
 */
var __provider = (() => {
  ...
  function searchAnime(titles) {
    return __async(this, null, function* () {
      for (const title of titles.slice(0, MAX_SEARCH_TITLES)) {
        try {
          const cleanTitle = stripSeasonSuffix(title);
          const html = yield postSearch(cleanTitle, { timeout: TIMEOUTS.SEARCH });
      ...
      for (const query of fallbackQueries) {
        try {
          const html = yield postSearch(query, { timeout: TIMEOUTS.SEARCH });
          const results = parseSearchResults(html);
          if (results.length === 0) continue;
          for (const r of results) {
            for (const title of titles.slice(0, MAX_SEARCH_TITLES)) {
  ...
  function extractMovie(tmdbId, titles, subType) {
    return __async(this, null, function* () {
      const match = yield searchAnime(titles);
      if (!match) {
  ...
  function extractSeries(tmdbId, mediaType, titles, season, episode, subType) {
    return __async(this, null, function* () {
      ...
      const directResult = yield extractEpisodeStreams(match, targetSeasonNum, targetEpisodeNums[0], subType);
      ...
      if (getSpinoffPenalty(match.title) > 0) {
        ...
        try {
          const matchPageHtml = yield fetchText(match.url, { timeout: TIMEOUTS.PAGE }).catch(() => "");
          if (matchPageHtml) {
            const $ = import_cheerio_without_node_native2.default.load(matchPageHtml);
            const matchSignificantWords = match.title.toLowerCase().split(/\s+/).filter((w) => w.length > 3);
            ...
            if (parentLink) {
              ...
              if (parentIdMatch && parentIdMatch[1] !== match.animeId) {
                ...
                const parentResult = yield extractEpisodeStreams(parentMatch, targetSeasonNum, targetEpisodeNums[0], subType);
      ...
      try {
        ...
        const seriesHtml = yield fetchText(seriesUrl, { timeout: TIMEOUTS.PAGE });
  ...
  var import_cheerio_without_node_native2, SPINOFF_KEYWORDS, SEASON_PATTERN;
  var init_extractor = __esm({
    "src/animesama-co/extractor.js"() {
      ...
      SEASON_PATTERN = /\/saison-(\d+)\.html/g;
    }
  });

  // src/animesama-co/index.js
  var require_index = __commonJS({
    "src/animesama-co/index.js"(exports, module) {
      init_extractor();
      init_resolvers();
