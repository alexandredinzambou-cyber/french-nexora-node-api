 ...
 * Generated: 2026-09-11T18:14:24.414464302Z
 */
var __provider = (() => {
  ...
  function searchSlugsScored(query) {
    return __async(this, null, function* () {
      try {
        const html = yield withTimeout(fetchText(`${BASE_URL}/template-php/defaut/fetch.php`, {
          method: "POST",
          headers: {
  ...
  function fetchJs(slug, seasonPath, lang) {
    return __async(this, null, function* () {
      ...
      try {
        const content = yield withTimeout(fetchText(url), 8e3, `fetchJs ${slug}`);
        return content || null;
      } catch (e) {
        return null;
      }
  ...
  function fetchAndGetUrl(_0, _1, _2, _3, _4) {
    return __async(this, arguments, function* (slug, lang, season, episode, mediaType, altEpisodes = []) {
      const episodesToTry = [episode, ...altEpisodes.filter((e) => e !== episode)];
      if (mediaType === "movie") {
        const jsContent = yield fetchJs(slug, "film", lang);
        if (!jsContent) return [];
      ...
      for (const ep of episodesToTry) {
        const result = yield tryFetchEpisode(slug, lang, season, ep);
  ...
  function extractStreams(tmdbId, mediaType, season, episode) {
    return __async(this, null, function* () {
      ...
      const episodes = yield resolveTargetEpisodes(tmdbId, mediaType, season, episode, { startTime, budgetMs: BUDGET_MS });
      ...
      if (streams.length === 0 && effectiveSeason > 1 && !isBudgetExhausted(startTime, BUDGET_MS)) {
        ...
        const seasonResults = yield Promise.all(seasonPromises);
      ...
      if (streams.length === 0 && effectiveSeason > 1 && !isBudgetExhausted(startTime, BUDGET_MS)) {
        ...
        const numResults = yield Promise.all(numPromises);
