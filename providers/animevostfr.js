 ...
 * Generated: 2026-09-11T18:14:24.613464564Z
 */
var __provider = (() => {
  ...
  var rateLimit, DOMAIN, HEADERS2, HTTP_SKIP_CODES;
  var init_http = __esm({
    "src/animevostfr/http.js"() {
      init_resolvers();
  ...
  function searchAnime(title) {
    return __async(this, null, function* () {
      try {
        const html = yield fetchText(`${BASE_URL}/?s=${encodeURIComponent(title)}`, { timeout: SEARCH_TIMEOUT });
        ...
        console.log(`[AnimeVOSTFR] Best match: "${best == null ? void 0 : best.title}" (score ${bestScore}) -> ${matches.length} results kept`);
  ...
  function extractPlayersFromEpisode(episodeUrl) {
    return __async(this, null, function* () {
      const streams = [];
      try {
        const html = yield fetchText(episodeUrl, { timeout: SEARCH_TIMEOUT });
        const $ = import_cheerio_without_node_native2.default.load(html);
        const serverNames = {};
        ...
        const trembedPromises = trembedEntries.map((entry) => __async(null, null, function* () {
          try {
            ...
            const embedHtml = yield fetchText(trembedUrl, { timeout: SEARCH_TIMEOUT, headers: { "Referer": episodeUrl } });
            const $embed = import_cheerio_without_node_native2.default.load(embedHtml);
        ...
        const playerStreams = yield Promise.all(trembedPromises);
  ...
  function extractStreams(tmdbId, mediaType, season, episode) {
    return __async(this, null, function* () {
      ...
      const mainTitle = ((_a = titlesOrdered[0]) == null ? void 0 : _a.toLowerCase()) || "";
      const mainWords = mainTitle.split(/\s+/).filter((w) => w.length > 3);
      const uniqueMatches = [];
      const seenMatchUrls = /* @__PURE__ */ new Set();
      ...
      const matchPromises = matchesToProcess.map((match) => __async(null, null, function* () {
        ...
        const epResults = yield Promise.allSettled(
          targetEpisodes.map((ep) => __async(null, null, function* () {
            const isAbsolute = ep !== searchEpisode;
            const episodeUrl = yield findEpisodeUrl(match.url, searchSeason, ep, isAbsolute);
      ...
      const results = yield Promise.allSettled(matchPromises);
  ...
  var import_cheerio_without_node_native2, BASE_URL, MAX_SEARCH_TITLES, SEARCH_TIMEOUT;
