 ...
 * Generated: 2026-09-11T18:14:25.09946505Z
 */
var __provider = (() => {
  ...
  function isCloudflareBlock(text) {
    return text && (text.includes("Cloudflare") || text.includes("cf-browser-verification") || text.includes("challenge-form") || text.includes("Attention Required") || text.includes("Just a moment...") || text.length < 200);
  }
  function fetchText(_0) {
    return __async(this, arguments, function* (url, options = {}) {
      var _b, _c, _d, _e, _f, _g;
      yield rateLimit(DOMAIN);
      const _a = options, { headers: customHeaders, retries = 2 } = _a, rest = __objRest(_a, ["headers", "retries"]);
      ...
      for (let attempt = 0; attempt <= retries; attempt++) {
        if (attempt > 0) {
          const delay = RETRY_DELAYS[attempt - 1] || 5e3;
        ...
        try {
          ...
          lastError = e;
          if (attempt < retries && (((_b = e.message) == null ? void 0 : _b.includes("fetch failed")) || ((_c = e.message) == null ? void 0 : _c.includes("NetworkError")) || ((_d = e.message) == null ? void 0 : _d.includes("timeout")) || ((_e = e.message) == null ? void 0 : _e.includes("no response")) || ((_f = e.message) == null ? void 0 : _f.includes("Cloudflare")) || ((_g = e.message) == null ? void 0 : _g.includes("429")))) {
  ...
  var rateLimit, DOMAIN, RETRY_DELAYS, HEADERS2;
  var init_http = __esm({
    "src/sekai/http.js"() {
      init_resolvers();
  ...
  function toSekaiSlug(title) {
    if (!title) return "";
    let slug = title.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[':!.,?'']/g, "").replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").replace(/-+/g, "-");
  ...
  function getSeriesSlugs() {
    return __async(this, null, function* () {
      return withCache("series_slugs_from_api", () => __async(null, null, function* () {
        try {
          const js = yield fetchText(`${BASE_URL}/episodesData.js`);
          const slugKeys = [];
  ...
  function extractSagaUrls(html, slug) {
    ...
    return [...sagas].sort((a, b) => {
      var _a, _b;
      const na = parseInt(((_a = a.match(/saga-(\d+)/)) == null ? void 0 : _a[1]) || "0");
      const nb = parseInt(((_b = b.match(/saga-(\d+)/)) == null ? void 0 : _b[1]) || "0");
      return na - nb;
    });
  }
  function getSeriesData() {
    return __async(this, null, function* () {
      const slugs = yield getSeriesSlugs();
      if (slugs.length === 0) return [];
      return slugs.map((s) => ({
        title: s.slug,
        url: s.url,
        aliases: [s.title]
        // le nom interne comme alias
      }));
    });
  }
  function fetchSagaPage(sagaUrl) {
    return __async(this, null, function* () {
      return withCache(`saga_page_${sagaUrl}`, () => __async(null, null, function* () {
        try {
          const html = yield fetchText(sagaUrl);
          return html && html.length > 1e3 ? html : "";
        } catch (e) {
          return "";
        }
      }), { successTtl: CACHE_TTL });
  ...
  function extractStreams(tmdbId, mediaType, season, episodeNum) {
    return __async(this, null, function* () {
      ...
      const [ep, absoluteEp] = yield resolveTargetEpisodes(tmdbId, mediaType, season, episodeNum, { startTime, budgetMs: BUDGET_MS });
      ...
      let mainHtml = "";
      try {
        mainHtml = yield fetchText(seriesUrl);
      } catch (e) {
        console.log(`[Sekai] Series page not found at ${seriesUrl}`);
      }
      if (mainHtml.length < 5e3) {
        for (const altTitle of titles.slice(1)) {
          ...
          try {
            mainHtml = yield fetchText(altUrl);
            if (mainHtml.length >= 5e3) {
              seriesUrl = altUrl;
        ...
        if (allSeries.length > 0) {
          ...
          for (const s of allSeries) {
            ...
            for (const t of titles) {
              ...
              if (score >= 70) {
                try {
                  mainHtml = yield fetchText(s.url);
                  if (mainHtml.length >= 5e3) {
                    seriesUrl = s.url;
      ...
      const actualSeriesUrl = mainHtml.length >= 5e3 && seriesUrl ? seriesUrl : null;
      const effectiveSlug = actualSeriesUrl ? actualSeriesUrl.split("/").pop().replace(/\?.*$/, "") : slug;
      if (!effectiveSlug) {
      ...
      if (!isBudgetExhausted(startTime, BUDGET_MS) && sagasToFetch.length > 0) {
        ...
        while (idx < sagasToFetch.length && !isBudgetExhausted(startTime, BUDGET_MS)) {
          ...
          const batchHtmls = yield Promise.all(
            batch.map((url) => fetchSagaPage(url).catch(() => ""))
          );
          for (const html of batchHtmls) {
            if (!html || html.length < 1e3) continue;
  ...
  var BASE_URL, BUDGET_MS, CACHE_TTL, withCache, SLUG_OVERRIDES;
  var init_extractor = __esm({
    "src/sekai/extractor.js"() {
      ...
      BUDGET_MS = 4e4;
      CACHE_TTL = 3e5;
      withCache = createCache("sk", "Sekai");
      SLUG_OVERRIDES = {
        "one-piece": "piece",
        "one piece": "piece",
        "dr-stone": "drstone",
