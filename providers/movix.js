 ...
 * Generated: 2026-09-11T18:14:24.939464891Z
 */
var __provider = (() => {
  ...
  var PROVIDER_BUDGET_MS, HEADERS, USER_AGENT, BASE_HEADERS, _atob, CODEC_PREFERENCE, STRICT_QUALITY_TIERS, DEFAULT_QUALITY_TIER, CODEC_PRIORITY, manifestCache, MANIFEST_CACHE_TTL, FETCH_CACHE_TTL, fetchCache, KNOWN_HOST_NAMES, peeledUrls, AD_IFRAME_PATTERNS, VIDEO_IFRAME_SCORE, BASE_URL_FORBIDDEN_PATTERN;
  ...
  function fetchJson(_0) {
    return __async(this, arguments, function* (url, options = {}) {
      ...
      for (let attempt = 0; attempt <= retries; attempt++) {
        try {
          const result = yield attemptFetch(url, customHeaders, rest);
      ...
      console.log(`[Movix] All ${retries + 1} attempts failed for ${url}: ${lastError == null ? void 0 : lastError.message}`);
      return null;
    });
  }
  function attemptFetch(url, customHeaders, rest) {
    return __async(this, null, function* () {
  ...
  function getUrlOrigin(url, fallback) {
    try {
  ...
  function fetchFromDomain(baseUrl, tmdbId, mediaType, season) {
    return __async(this, null, function* () {
      ...
      const path = isMovie ? `${baseUrl}/api/fstream/movie/${tmdbId}` : `${baseUrl}/api/fstream/tv/${tmdbId}/season/${Number(season) || 1}`;
  ...
  function parseStreams(data, provider, isMovie, episodeNum) {
    ...
    if (data.players) {
      for (const lang of Object.keys(data.players)) {
        ...
        for (const item of list) {
          pushStream(streams, provider, item == null ? void 0 : item.player, lang, item == null ? void 0 : item.url, item == null ? void 0 : item.quality);
        }
      }
    }
    if (data.links) {
      for (const lang of Object.keys(data.links)) {
        ...
        if (!Array.isArray(list)) continue;
        for (const item of list) {
          pushStream(streams, provider, (item == null ? void 0 : item.name) || (item == null ? void 0 : item.player), lang, item == null ? void 0 : item.url, item == null ? void 0 : item.quality);
        }
      }
    }
    ...
    for (const lang of ["vf", "vostfr", "vo", "VFF", "VFQ"]) {
      ...
      if (!Array.isArray(list)) continue;
      for (const item of list) {
        pushStream(streams, provider, (item == null ? void 0 : item.player) || (item == null ? void 0 : item.name), lang, item == null ? void 0 : item.url, item == null ? void 0 : item.quality);
      }
    }
    return streams;
  }
  ...
  function searchFallback(baseUrl, tmdbId, mediaType, season, episode) {
    return __async(this, null, function* () {
      ...
      try {
        const searchData = yield fetchJson(searchUrl, { retries: 0 });
        if (!(searchData == null ? void 0 : searchData.results) && !Array.isArray(searchData)) {
        ...
        for (const result of results.slice(0, 5)) {
          ...
          const path = resultType === "movie" ? `${baseUrl}/api/fstream/movie/${altId}` : `${baseUrl}/api/fstream/tv/${altId}/season/${Number(season) || 1}`;
          const altData = yield fetchJson(path, { retries: 0 });
          if (altData) {
  ...
  function extractStreams(tmdbId, mediaType, season, episode) {
    return __async(this, null, function* () {
      ...
      for (const baseUrl of API_DOMAINS) {
        ...
        const result = yield fetchFromDomain(baseUrl, tmdbId, mediaType, season);
        if (result) {
      ...
      if (allStreams.length === 0) {
        ...
        for (const baseUrl of API_DOMAINS) {
          const fallbackStreams = yield searchFallback(baseUrl, tmdbId, mediaType, season, episode);
  ...
  var API_DOMAINS, SLOW_HOSTS, FAST_HOSTS;
