 ...
 * Generated: 2026-09-11T18:14:25.227465184Z
 */
var __provider = (() => {
  ...
  function expandSingleStreamQualities(_0) {
    return __async(this, arguments, function* (stream, options = {}) {
      ...
      if (!options.forceRefresh) {
        const cached2 = getCachedManifest(cacheKey);
  ...
  function safeFetch(_0) {
    return __async(this, arguments, function* (url, options = {}) {
      ...
      if (method === "GET") {
        const cached2 = getCachedFetch(cacheKey);
        if (cached2) {
          return {
            text: () => Promise.resolve(cached2.bodyText),
            json: () => __async(null, null, function* () {
              try {
                return JSON.parse(cached2.bodyText);
              } catch (e) {
                throw e;
              }
            }),
            ...
            url: cached2.finalUrl || url,
            headers: cached2.headers || {}
          };
        }
      }
      try {
  ...
  // src/voiranime-rip/config.js
  var SITE, PATTERNS, TIMEOUTS, SCORES, CACHE_TTL, MAX_SEARCH_TITLES;
  ...
  // src/voiranime-rip/http.js
  function fetchText(_0) {
    return __async(this, arguments, function* (url, options = {}) {
      var _a, _b;
      yield rateLimit(DOMAIN);
      const timeout = (_a = options.timeout) != null ? _a : TIMEOUTS.PAGE;
      const mergedHeaders = __spreadValues(__spreadValues({}, HEADERS2), options.headers || {});
      const retries = (_b = options.retries) != null ? _b : 2;
      return fetchWithRetry(() => __async(null, null, function* () {
        const res = yield safeFetch(url, { headers: mergedHeaders, timeout });
  ...
  function postSearch(_0) {
    return __async(this, arguments, function* (query, options = {}) {
      yield rateLimit(DOMAIN);
      const sanitized = sanitizeSearchQuery(query);
      const body = `query=${encodeURIComponent(sanitized)}`;
      const mergedHeaders = __spreadValues(__spreadProps(__spreadValues({}, AJAX_HEADERS), {
        "Content-Type": "application/x-www-form-urlencoded"
      }), options.headers || {});
      return fetchWithRetry(() => __async(null, null, function* () {
        var _a;
        const res = yield safeFetch(`${SITE.BASE_URL}/template-php/defaut/fetch.php`, {
          method: "POST",
          headers: mergedHeaders,
          body,
          timeout: (_a = options.timeout) != null ? _a : TIMEOUTS.SEARCH
        });
  ...
  var rateLimit, DOMAIN, HEADERS2, AJAX_HEADERS;
  var init_http = __esm({
    "src/voiranime-rip/http.js"() {
      init_resolvers();
      init_config();
  ...
  var init_metadata = __esm({
    "src/utils/metadata.js"() {
      init_resolvers();
      TMDB_API_KEY = "f116bcfb16113ac02d873ce3e5f2820b";
  ...
  function scoreMatch(resultTitle, searchTitle, SCORES2) {
    const nt = normalize(searchTitle);
    const nr = normalize(resultTitle);
    if (!nt || !nr) return 0;
    if (nr === nt) return SCORES2.EXACT_MATCH;
    if (nr.includes(nt) || nt.includes(nr)) return SCORES2.STRONG_MATCH;
    ...
    return __async(this, null, function* () {
      try {
        ...
        if (resolved && resolved.url) {
          ...
          return __spreadProps(__spreadValues({}, resolved), { isDirect: true });
        }
        return null;
      } catch (e) {
        return null;
  ...
  function detectSubType(tmdbId, mediaType) {
    return __async(this, null, function* () {
      const apiKey = "8265bd1679663a7ea12ac168da84d2e8";
      const type = mediaType === "movie" ? "movie" : "tv";
      try {
        const details = yield cached(`tmdb_subtype_${tmdbId}_${mediaType}`, () => __async(null, null, function* () {
  ...
  // src/voiranime-rip/extractor.js
  ...
  function detectLanguage(html) {
    ...
    const body = $("body").text().toLowerCase().slice(0, 5e3);
  ...
  function searchAnime(titles) {
    return __async(this, null, function* () {
      for (const title of titles.slice(0, MAX_SEARCH_TITLES)) {
        try {
          const result = yield trySearchQuery(title, false, titles);
      ...
      for (const query of fallbackQueries) {
        try {
          const result = yield trySearchQuery(query, true, titles);
          if (result) {
      ...
      if (firstTitle) {
        ...
        const normalizeForSlug = (t) => t.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[':!.,?()\[\]]/g, "").replace(/[^a-z0-9\s]/g, "").trim();
        ...
        if (compactedSlugs.length > 0) {
          ...
          for (const slug of uniqueSlugs) {
            try {
              ...
              const html = yield fetchText(url, { timeout: 3e3 });
  ...
  function trySearchQuery(query, isFallback = false, originalTitles = null) {
    return __async(this, null, function* () {
      const cleanQuery = stripSeasonSuffix(query);
      const html = yield postSearch(cleanQuery, { timeout: TIMEOUTS.SEARCH });
      const results = parseSearchResults(html);
      if (results.length === 0) return null;
      let scored = results.map((r) => __spreadProps(__spreadValues({}, r), { score: scoreMatch(r.title, query, SCORES) + getSpinoffPenalty(r.title) })).filter((r) => r.score >= SCORES.MIN_MATCH).sort((a, b) => b.score - a.score);
  ...
  function detectSubTypeCached(tmdbId, mediaType) {
    return __async(this, null, function* () {
  ...
  function extractStreams(tmdbId, mediaType, season, episode) {
    return __async(this, null, function* () {
      ...
      const subType = yield detectSubTypeCached(tmdbId, mediaType);
  ...
  function extractMovie(tmdbId, titles, subType) {
    return __async(this, null, function* () {
      ...
      for (const match of matches) {
        const result = yield extractMoviePageStreams(match, subType);
        if (result.length > 0) return result;
      }
      return [];
    });
  }
  function extractMoviePageStreams(match, subType) {
    return __async(this, null, function* () {
      console.log(`[VoiranimeRip] Fetching movie page: ${match.url}`);
      try {
        const html = yield fetchText(match.url, { timeout: TIMEOUTS.PAGE });
        if (!html) {
        ...
        for (const v of videoUrls) {
          ...
          if (resolved && resolved.url) {
            ...
            streams.push(__spreadProps(__spreadValues({}, resolved), { provider: "voiranime-rip" }));
          }
        }
        if (streams.length === 0) {
          for (const v of videoUrls) {
            ...
            const stream = toStream(v.url, lang, "Voiranime-Rip", SITE.BASE_URL);
            if (subType) stream.subType = subType;
            streams.push(__spreadProps(__spreadValues({}, stream), { provider: "voiranime-rip", isDirect: false }));
          }
        }
  ...
  function extractSeries(tmdbId, mediaType, titles, season, episode, subType) {
    return __async(this, null, function* () {
      ...
      for (const match of matches) {
        const result = yield extractEpisodeStreams(match, targetSeasonNum, parseInt(episode) || 1, subType);
        if (result.length > 0) {
  ...
  var import_cheerio_without_node_native2, SEASON_PATTERN, withCache, SPINOFF_KEYWORDS;
  var init_extractor = __esm({
    "src/voiranime-rip/extractor.js"() {
      ...
      withCache = createCache("vr", "VoiranimeRip");
      SPINOFF_KEYWORDS = ["fan letter", "log:", "memories", "vigilante", "illegals", "film", "movie", "special", "oav", "ona", "x ut", "collab", "junior high", "chibi", "super deformed"];
    }
  });

  // src/voiranime-rip/index.js
  var require_index = __commonJS({
    "src/voiranime-rip/index.js"(exports, module) {
      init_extractor();
      init_resolvers();
